...
def test_can_apply_create(initial_state):
    ...
    assert MovementRules.can_apply(mov, initial_state._paths) is True

    mov_existing = {"op_type": "CREATE", "new_rel_path": "file1.txt"}
    # file1.txt ya existe → no se puede crear
    assert MovementRules.can_apply(mov_existing, initial_state._paths) is False
...
def test_can_apply_modify(initial_state):
    ...
    assert MovementRules.can_apply(mov_nonexistent, initial_state._paths) is False
...
def test_can_apply_move(initial_state):
    ...
    assert MovementRules.can_apply(mov_target_exists, initial_state._paths) is False
    ...
    assert MovementRules.can_apply(mov_source_missing, initial_state._paths) is False
...
def test_can_apply_unknown(initial_state):
    ...
    assert MovementRules.can_apply(mov, initial_state._paths) is False
